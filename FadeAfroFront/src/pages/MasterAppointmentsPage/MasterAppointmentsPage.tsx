...
import { getMasterAppointments, cancelMasterAppointment } from '../../api/appointments'
...
import styles from './MasterAppointmentsPage.module.css'
...
const AppointmentCard = ({
  ...
  const [cancelState, setCancelState] = useState<'idle' | 'loading' | 'success'>('idle')
    ...
    <div ref={ref} className={styles.card}>
      ...
      <div className={styles.section}>
        ...
        <div className={styles.sectionInfo}>
          <span className={styles.sectionTitle}>{clientName}</span>
          <span className={styles.sectionSub}>Клиент</span>
        </div>
      </div>
      ...
      {appointment.comment && (
        <>
          <div className={styles.divider} />
          <div className={styles.section}>
            <div className={styles.iconCircle}><CommentIcon /></div>
            <div className={styles.sectionInfo}>
              <span className={styles.sectionTitle}>{appointment.comment}</span>
            </div>
          </div>
        </>
      )}
      ...
      {/* Отмена */}
      {cancelState === 'loading' && (
        <div className={styles.cancelBtnLoading}>
          <span /><span /><span />
        </div>
      )}
      {cancelState === 'success' && (
        <div className={styles.cancelBtnSuccess}>
          <CheckIcon />
        </div>
      )}
      {cancelState === 'idle' && (
    ...
    <div className={styles.page}>
      ...
      {appointments.length === 0 ? (
        <div className={styles.emptyCard}>
          ...
          <p className={styles.emptyText}>Здесь появятся записи клиентов</p>
        </div>
      ) : (
